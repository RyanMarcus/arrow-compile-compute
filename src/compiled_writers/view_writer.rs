...
impl ViewBufferWriter {
    ...
    pub fn write(&mut self, data: &[u8]) -> u128 {
        ...
        let offset = if last_buf.capacity() >= data.len() {
            ...
            let next_size = usize::max(next_size, data.len());
    ...
    pub fn into_buffers(self) -> Vec<arrow_buffer::Buffer> {
        self.buffers
            .into_iter()
            .map(arrow_buffer::Buffer::from)
            .collect()
    }
}
...
mod tests {
    ...
    fn string_view_writer_jit_writes_inline_and_buffered_values_and_reserves() {
        ...
        let llvm_mod = ctx.create_module("compiled_writers_string_view_writer");
