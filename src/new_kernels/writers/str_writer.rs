...
use crate::new_kernels::writers::EWriterAllocation;
...
pub struct EStringArrayWriter<'a, T: OffsetSizeTrait> {
...
#[derive(ReprOffset)]
#[roff(usize_offsets)]
pub struct EStringAllocation<T: OffsetSizeTrait> {
    offsets_ptr: *mut c_void,
    offsets: Vec<T>,
    data: Vec<u8>,
}

impl<T: OffsetSizeTrait> EStringAllocation<T> {
    pub fn get_ptr(&mut self) -> *mut c_void {
        self as *mut Self as *mut c_void
    }

    pub fn to_array(mut self, len: usize, nulls: Option<arrow_buffer::NullBuffer>) -> ArrayRef {
        ...
        Arc::new(GenericByteArray::<GenericBinaryType<T>>::new(
            offsets, data, nulls,
        ))
    }
}

impl From<EStringAllocation<i32>> for EWriterAllocation {
    fn from(output: EStringAllocation<i32>) -> Self {
        EWriterAllocation::String(output)
    }
}

impl From<EStringAllocation<i64>> for EWriterAllocation {
    fn from(output: EStringAllocation<i64>) -> Self {
        EWriterAllocation::LargeString(output)
    }
}

impl<'a, T: OffsetSizeTrait> EStringArrayWriter<'a, T> {
    pub fn allocate(expected_count: usize) -> EStringAllocation<T> {
    ...
    pub fn llvm_ingest(&self, _ctx: &'a Context, build: &Builder<'a>, val: BasicValueEnum<'a>) {
        build
            .build_call(
                self.ingest_func,
                &[
    ...
    pub fn llvm_flush(&self, _ctx: &'a Context, _build: &Builder<'a>) {
