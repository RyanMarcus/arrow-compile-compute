...
#[roff(usize_offsets)]
pub struct EBooleanAllocation {
    data_ptr: *mut c_void,
    data: Vec<u8>,
}

impl EBooleanAllocation {
    pub fn get_ptr(&mut self) -> *mut c_void {
        self as *mut Self as *mut c_void
    }

    pub fn to_array(self, len: usize, nulls: Option<arrow_buffer::NullBuffer>) -> ArrayRef {
        let buf = Buffer::from(self.data);
        let bb = BooleanBuffer::new(buf, 0, len);
        Arc::new(BooleanArray::new(bb, nulls))
    }
}

impl<'a> EBooleanWriter<'a> {
    fn allocate(expected_count: usize, ty: PrimitiveType) -> EBooleanAllocation {
    ...
    pub fn llvm_flush(&self, _ctx: &'a Context, build: &Builder<'a>) {
        build
            .build_call(
                self.flush_func,
                &[
