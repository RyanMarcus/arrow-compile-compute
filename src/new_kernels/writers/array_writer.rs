...
use crate::{
    declare_blocks, increment_pointer, new_kernels::writers::EWriterAllocation, PrimitiveType,
};
...
pub struct EPrimitiveArrayWriter<'a> {
...
impl EArrayOutput {
    ...
    pub fn to_array(self, len: usize, nulls: Option<NullBuffer>) -> ArrayRef {
...
impl From<EArrayOutput> for EWriterAllocation {
    fn from(output: EArrayOutput) -> Self {
        EWriterAllocation::Primitive(output)
    }
}

impl<'a> EPrimitiveArrayWriter<'a> {
    pub fn allocate(expected_count: usize, ty: PrimitiveType) -> EArrayOutput {
