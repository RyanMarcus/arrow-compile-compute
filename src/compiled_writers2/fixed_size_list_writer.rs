...
mod tests {
    ...
    fn fixed_size_list_writer_composes_primitive_and_reserves() {
        ...
        let llvm_mod = ctx.create_module("compiled_writers2_fixed_size_list_primitive");
    ...
    fn fixed_size_list_writer_composes_boolean_writer() {
        let ctx = Context::create();
        let llvm_mod = ctx.create_module("compiled_writers2_fixed_size_list_boolean");
        let build = ctx.create_builder();
        let ptr_type = ctx.ptr_type(AddressSpace::default());
        let func = llvm_mod.add_function(
            "test",
    ...
    fn fixed_size_list_writer_composes_string_writer() {
        let ctx = Context::create();
        let llvm_mod = ctx.create_module("compiled_writers2_fixed_size_list_string");
        let build = ctx.create_builder();
        let ptr_type = ctx.ptr_type(AddressSpace::default());
        let func = llvm_mod.add_function(
            "test",
