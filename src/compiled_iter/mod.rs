...
pub fn generate_random_access<'a>(
    ...
    match ih {
        ...
        IteratorHolder::ScalarPrimitive(iter) => {
            ...
            build.position_at_end(entry);
            let val_ptr = iter.llvm_val_ptr(ctx, &build, iter_ptr);
            let raw_val = build
                .build_load(ctx.i64_type(), val_ptr, "scalar_raw")
                .unwrap()
                .into_int_value();
            ...
            let result = match ptype {
                F16 => {
                    let bits = build
                        .build_int_truncate(raw_val, ctx.i16_type(), "scalar_f16_bits")
                        .unwrap();
                    build
                        .build_bit_cast(bits, llvm_type.into_float_type(), "scalar_f16")
                        .unwrap()
                }
                F32 => {
                    let bits = build
                        .build_int_truncate(raw_val, ctx.i32_type(), "scalar_f32_bits")
                        .unwrap();
                    build
                        .build_bit_cast(bits, llvm_type.into_float_type(), "scalar_f32")
                        .unwrap()
                }
                F64 => build
                    .build_bit_cast(raw_val, llvm_type.into_float_type(), "scalar_f64")
                    .unwrap(),
                I8 | I16 | I32 | I64 | U8 | U16 | U32 | U64 => {
                    let int_type = llvm_type.into_int_type();
                    if int_type.get_bit_width() < 64 {
                        build
                            .build_int_truncate(raw_val, int_type, "scalar_trunc")
                            .unwrap()
                            .as_basic_value_enum()
                    } else {
                        raw_val.as_basic_value_enum()
                    }
                }
                P64x2 => unreachable!("string scalar should use ScalarString iterator"),
            };

            build.build_return(Some(&result)).unwrap();
            Some(access_f)
        }
        IteratorHolder::ScalarString(iter) => {
            let ret_type = PrimitiveType::P64x2.llvm_type(ctx).into_struct_type();
            let ptr_type = ctx.ptr_type(AddressSpace::default());
            ...
            build.position_at_end(entry);
            let (ptr1_ptr, ptr2_ptr) = iter.llvm_val_ptr(ctx, &build, iter_ptr);
            let ptr1 = build
                .build_load(ptr_type, ptr1_ptr, "scalar_str_ptr1")
                .unwrap()
                .into_pointer_value();
            let ptr2 = build
                .build_load(ptr_type, ptr2_ptr, "scalar_str_ptr2")
                .unwrap()
                .into_pointer_value();
            let to_return = ret_type.const_zero();
            let to_return = build
                .build_insert_value(to_return, ptr1, 0, "scalar_str_ret1")
                .unwrap();
            let to_return = build
                .build_insert_value(to_return, ptr2, 1, "scalar_str_ret2")
                .unwrap();
            build.build_return(Some(&to_return)).unwrap();
            Some(access_f)
        }
        IteratorHolder::ScalarBinary(iter) => {
            let ret_type = PrimitiveType::P64x2.llvm_type(ctx).into_struct_type();
            let ptr_type = ctx.ptr_type(AddressSpace::default());
            ...
            let (ptr1_ptr, ptr2_ptr) = iter.llvm_val_ptr(ctx, &build, iter_ptr);
            let ptr1 = build
                .build_load(ptr_type, ptr1_ptr, "scalar_bin_ptr1")
                .unwrap()
                .into_pointer_value();
            let ptr2 = build
                .build_load(ptr_type, ptr2_ptr, "scalar_bin_ptr2")
                .unwrap()
                .into_pointer_value();
            let to_return = ret_type.const_zero();
            let to_return = build
                .build_insert_value(to_return, ptr1, 0, "scalar_bin_ret1")
                .unwrap();
            let to_return = build
                .build_insert_value(to_return, ptr2, 1, "scalar_bin_ret2")
                .unwrap();
            build.build_return(Some(&to_return)).unwrap();
            Some(access_f)
        }
