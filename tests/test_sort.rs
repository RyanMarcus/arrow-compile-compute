...
proptest! {
    ...
    fn test_lower_bound_proptest(
        ...
        let key_input = (&key_array as &dyn arrow_array::Datum, SortOptions::default());

        let result = arrow_compile_compute::sort::lower_bound(key_input, &sorted_array)
            .unwrap();
...
fn test_lower_bound_single_column() {
    ...
    let key_input = (&keys as &dyn arrow_array::Datum, SortOptions::default());
...
#[test]
fn test_lower_bound_string() {
    let sorted = StringArray::from(vec!["a", "b", "b", "d"]);
    let keys = StringArray::from(vec!["", "b", "c", "z"]);
...
#[test]
fn test_lower_bound_mixed_numeric_types() {
    let sorted = Int64Array::from(vec![1_i64, 3, 5, 7]);
    let keys = Int32Array::from(vec![0_i32, 3, 4, 8]);
...
#[test]
fn test_topk_indices_full_len() {
    let array = Int32Array::from(vec![5, 1, 4, 2]);
    let arr_refs: [&dyn Array; 1] = [&array];
    let options = [SortOptions::default()];

    let result = arrow_compile_compute::sort::topk_indices(&arr_refs, &options, array.len())
        .unwrap()
        .into_iter()
        .map(|x| x.unwrap())
        .collect_vec();
